...
**IT HAS NOT HAPPENED HERE, AND CANNOT YET.** This repository has a
CHANGELOG.md, written by hand, but carries no tags and no `chore(main): release`
commit — release-please has never cut a release here, so there is nothing for it
to have duplicated. This is installed *ahead* of the problem, on the reasoning
that corpus-lens merges pull requests with merge commits (see `git log` on main)
and the duplication becomes immediate the moment release-please cuts its first
section.

Said plainly, because it is the honest status: the behaviour of this file is
verified by its tests and by its identical twins in forge-play, willow-mcp and
jeles, and **not** by having corrected a real changelog here. Its first live run
will be its first live run.

PORTED FILE. This is forge-play/Forge's `tools/changelog_dedup.py`, copied
deliberately rather than reimplemented — divergent copies of a release
mechanism are how one repo silently stops publishing. Everything below this
docstring is byte-for-byte with that original; only this status paragraph is
rewritten, because the original's described the Forge repo (no CHANGELOG.md,
tags v0.0.3–v0.0.9) and neither half of that is true here. Fix the tool in
forge-play first, then re-sync.
