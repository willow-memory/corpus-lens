"""corpuslens.consent.core — consent for a subject who isn't the owner.

VENDORED, byte-for-byte from the ``from __future__`` line onward, from
willow-mcp's ``src/willow_mcp/subject_consent/core.py`` (Apache-2.0), which is
itself vendored from the canonical rudi193-cmd/safe-app-store
``libs/subject-consent`` (MIT). Canonical lives there; this copy is not a place
to edit. ``tests/test_consent.py`` pins the code body's hash to the SAME value
willow-mcp's own ``tests/test_subject_consent.py`` pins, so the two copies can
be checked against each other without either repo importing the other.

Why this file exists here at all. corpuslens draws a hard line at
``owner == subject``: pointing it at another person's corpus is out of scope
by design, and IDEAS.md names the guardian-consent model (owner != subject)
the biggest gap between this toolkit and any family-facing instrument. This
core is the fleet's one shared answer to the half of that gap that is
representable: *did this person — or a guardian on their behalf — agree to
their data being used this way?* It records grants; it does not judge who
may give one (age, capacity, relation) — that policy is deferred, here as in
every other consumer, to a human.

What corpuslens wires it to, and what it deliberately does not (see
``corpuslens/subject_consent.py`` for the binding):

  * ``--subject ID --consent-store DIR`` on ``run`` names a non-owner subject.
    The run proceeds only if ``permitted(store, ID, "process_analysis")`` is a
    verified GRANTED — fail-closed on absence, a broken chain, ``pending``,
    ``revoked``. The run then appends one row to that subject's disclosure
    chain saying a process analysis ran, so the guardian's readable record
    has it. Without ``--subject`` nothing changes: every run today is the
    owner's own corpus, and the gate is not consulted.
  * ``person_inference`` is NOT wired. The core has a scope by that name and
    the Guard has a capability by that name, on purpose the same name — but a
    consent grant for it still does not let a PERSON_CLAIM_TYPES analyzer run
    here: the default registry may never grant that capability, and consent
    is necessary for a person-shaped claim about someone, not sufficient. A
    grant answers "may this be about them"; it does not make the claim true,
    measured, or safe to emit. That wiring is a separate design act.

The core is stdlib-only, egress-free, and imports nothing from corpuslens —
which is what lets this stdlib-only-by-charter package take it at all.

Original willow-mcp docstring follows, kept verbatim for the record:
