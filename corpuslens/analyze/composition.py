...
@register(
    "composition_mix",
    claims=("composition_mix",),
    denominator="operator prompt turns with >=12 characters (de-injected)",
    version=1,
    grading_question=(
        "question 2 (authored_code_pct / code_ref_pct, against the domain "
...
def composition_mix(events):
    ...
    def pct(k):
        return round(100 * sum(1 for f in turns if f.get(k)) / n, 1)
    ...
    return {
        "headline": (
            f"You authored code in {authored}% of your prompts and referred to existing "
        ...
        "reference": REFERENCE,
        "reading": (
            "above the coding population on authored/read-ref = you bring the code to the "
...
@register(
    "clarification_pull",
    claims=("clarification_pull",),
    denominator="machine response turns (>=12 chars)",
    version=1,
    grading_question=(
        "none of GRADING.md's numbered questions directly — closest is "
...
def clarification_pull(events):
                ...
                if e.features.get("question"):
                    ...
                    nxt = next(
                        (
                            ...
                            if turns[j].author_class is AuthorClass.OPERATOR
                        ),
                        None,
                    )
                    if nxt is not None:
    ...
    if not asst:
        return {
            "error": "no machine responses in corpus — this analyzer needs a claude-code "
    ...
    return {
        "headline": (
            f"The machine asked a clarifying question you then answered in "
            f"{fork_pct}% of its turns."
        ),
        "n": asst,
        "assistant_turns": asst,
        "clarification_forks_pct": fork_pct,
        "reference": {
            "measured_cli": 3.4,
