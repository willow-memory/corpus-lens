...
def _turn(word_count, want_label, char_count=None, thread_id="t", day_offset=0, **extra_features):
    ...
    return Event(event_id=f"e{i}", corpus_id="c", adapter_id="test", source_ref=f"r{i}",
                 thread_id=thread_id, surface=Surface.CLI, author_class=AuthorClass.OPERATOR,
                 data_type=DataType.PROMPT, time=CoarseTime(day_offset=day_offset), features=feats)
...
class RegistrationTests(unittest.TestCase):
    def test_registered_with_a_process_claim(self):
        ...
        for claim in a.claims:
            self.assertIn(claim, PROCESS_CLAIM_TYPES,
...
class SingleSourceTests(unittest.TestCase):
    ...
    def test_one_uniform_human_source_reports_one_signature(self):
        events = [_turn(wc, sig_mod.HUMAN)
                  for wc in ([3, 4, 5, 40, 80, 150] * 10)][:SMALL_N + 10]
        res = _run(events)
...
class NoAttributionTests(unittest.TestCase):
    ...
    def test_output_never_stamps_a_signature_with_the_contract_vocabulary(self):
        ...
        res = _run(events)
        for entry in res["signatures"]:
            self.assertNotIn(entry["typical_length_band"],
                             (sig_mod.HUMAN, sig_mod.AGENT, sig_mod.UNKNOWN))
    ...
    def test_order_does_not_track_share_or_seniority(self):
        ...
        long_index_a = next(s["index"] for s in res_a["signatures"]
                            if s["typical_length_band"] == "long")
        long_index_b = next(s["index"] for s in res_b["signatures"]
                            if s["typical_length_band"] == "long")
        self.assertEqual(long_index_a, long_index_b,
