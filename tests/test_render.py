...
class HeadlineTests(unittest.TestCase):
    def test_findings_list_leads_the_report(self):
        md = markdown(
            {
                "a": {
                    "headline": "You did the thing 40% of the time.",
                    "n": 100,
                    "denominator": "turns",
                    "x": 1,
                }
    ...
    def test_headline_and_denominator_appear_in_the_section(self):
        md = markdown(
            {
                "a": {
                    "headline": "Forty percent.",
                    "n": 100,
                    "denominator": "turns with >=12 characters",
                }
            },
            _audit(),
        )
...
class RubricScopeTests(unittest.TestCase):
    ...
    def test_register_rejects_a_blank_grading_question(self):
        ...
        with self.assertRaises(ValueError):
            register(
                "bad", claims=("tempo",), denominator="turns", version=1, grading_question="  "
            )(lambda ev: {})
...
class ReferenceProvenanceTests(unittest.TestCase):
    ...
    def test_the_n1_reference_is_labelled_one_person(self):
        res = {"headline": "h", "n": 1, "reference": {"measured_director": "96.8% mid-task"}}
    ...
    def test_population_aggregates_are_not_relabelled_as_n1(self):
        res = {
            ...
            "n": 1,
            "reference": {
                "wildchat_coding_population": {"authored_pct": 14.5},
        ...
        wildchat_line = next(
            line for line in md.splitlines() if "wildchat_coding_population" in line
        )
        oasst_line = next(line for line in md.splitlines() if "oasst_general_chat" in line)
...
class NotComputableTests(unittest.TestCase):
    def test_error_is_stated_in_words_not_an_empty_section(self):
        md = markdown(
            {
                "tempo": {
                    "denominator": "turns with a delta",
                    "error": "no within-day tempo deltas in this corpus",
                }
            },
            _audit(),
        )
...
class NoSecondSetOfNumbersTests(unittest.TestCase):
    ...
    def test_only_already_rendered_strings_are_omitted(self):
        md = markdown(
            {"a": {"headline": "H-TEXT", "reading": "R-TEXT", "n": 40, "denominator": "turns"}},
            _audit(),
        )
...
class AnalyzerContractTests(unittest.TestCase):
    ...
    def setUp(self):
        ...
        _write(
            ...
            [
                ...
                _cc_line(
                    "user",
                    "lets talk about the tradeoffs for the cache layer",
    ...
    def test_every_analyzer_returns_a_headline_and_an_n(self):
        for a in all_analyzers():
            with self.subTest(analyzer=a.name):
                ...
                self.assertTrue(
                    res["headline"].strip().endswith("."), f"{a.name}'s headline is not a sentence"
                )
    ...
    def test_the_rendered_report_carries_no_content_or_anchor(self):
        results = {
            a.name: {"denominator": a.denominator, **a.run(self.events)} for a in all_analyzers()
        }
...
class PackagingTests(unittest.TestCase):
    ...
    def test_the_distribution_name_matches_everywhere_it_is_written(self):
        ...
        self.assertIn('_version("willow-corpus-lens")', self._repo_file("corpuslens/__init__.py"))
