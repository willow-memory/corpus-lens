...
ROWS = [
    ...
    (
        ...
        "user",
        "<system-reminder>x</system-reminder> what does mastery.py return?",
        "s1",
    ),
...
def _make_sqlite(path, cols=("created_at", "author", "body", "session_id"), table="turns"):
    ...
    con.execute(
        f'CREATE TABLE "{table}" ({cols[0]} TEXT, {cols[1]} TEXT, {cols[2]} TEXT, {cols[3]} TEXT)'
    )
...
class RowHelperTests(unittest.TestCase):
    ...
    def test_column_alias_resolution(self):
        ...
        self.assertEqual(
            (m["ts"], m["role"], m["content"], m["session"]),
            ("Created_At", "Author", "Body", "Session_Id"),
        )
...
class SqliteAdapterTests(unittest.TestCase):
    ...
    def test_cross_midnight_delta_censored(self):
        ...
        self.assertIn(
            300.0, [e.time.delta_prev_s for e in events if e.time.delta_prev_s is not None]
        )
...
class PostgresAdapterTests(unittest.TestCase):
    def setUp(self):
        ...
        subprocess.run(["createdb", self.dbname], check=True, capture_output=True, text=True)
        ...
        ddl = "CREATE TABLE turns (created_at text, author text, body text, session_id text);"
        subprocess.run(
            ["psql", self.dbname, "-X", "-q", "-c", ddl], check=True, capture_output=True, text=True
        )
        import csv
        ...
        for r in ROWS:
            w.writerow(r)
        subprocess.run(
            ["psql", self.dbname, "-X", "-q", "-c", "COPY turns FROM STDIN WITH (FORMAT csv)"],
            input=buf.getvalue(),
            check=True,
            capture_output=True,
            text=True,
    ...
    def tearDown(self):
        subprocess.run(["dropdb", "--if-exists", self.dbname], capture_output=True, text=True)
    ...
    def test_real_timestamptz_column_keeps_within_day_tempo(self):
        ...
        subprocess.run(
            [
                ...
                "-q",
                "-c",
                "CREATE TABLE tt (created_at timestamptz, author text, body text, session_id text)",
            ],
            check=True,
            capture_output=True,
            text=True,
    ...
    def test_malicious_table_name_cannot_inject(self):
        ...
        subprocess.run(
            ["psql", self.dbname, "-X", "-q", "-c", "CREATE TABLE canary (x int)"],
            check=True,
            capture_output=True,
            text=True,
        )
        ...
        still = subprocess.run(
            ["psql", self.dbname, "-X", "-tAc", "SELECT to_regclass('public.canary') IS NOT NULL"],
            capture_output=True,
            text=True,
        )
...
class ForeignFormatRoleTests(unittest.TestCase):
    ...
    def test_an_oasst_shaped_table_reads_end_to_end(self):
        ...
        con.execute(
            "CREATE TABLE turns (created_date TEXT, role TEXT, text TEXT, message_tree_id TEXT)"
        )
        con.executemany(
            "INSERT INTO turns VALUES (?,?,?,?)",
            [
