...
class DogfoodRegressions(unittest.TestCase):
    ...
    def test_local_command_replay_is_not_three_operator_turns(self):
        """Running `/model` locally puts three machine turns in the user role.

        Found 2026-09-11 by running corpuslens on this session's own log. The
        caveat block, the command echo and the command's stdout were 3 of 11
        "operator" turns; two carried backticks, which fired CODE_REF and
    ...
    def test_stop_hook_output_is_not_a_prompt(self):
        """A stop hook's stdout arrives in the user role with no wrapper at all.

        Same session, same finding: it is the hook reporting on the repository,
