...
def _authorship_stub(result: dict):
    return Analyzer(name="authorship_mix", claims=("authorship_mix",),
                     denominator="operator-role turns", version=1,
...
class NoAuthorshipAnalyzerTests(unittest.TestCase):
    """The realistic state of THIS branch: `authorship_mix` is not registered
    at all, because `corpuslens/authorship.py` has not landed."""
    ...
    def test_subject_defaults_to_unknown_and_says_why(self):
        ...
        self.assertIn("not registered", doc["audit"]["subject_reason"])
