...
def _make_store(root: Path, rel: str, blobs, created_ms=START_MS, meta_json=True):
    ...
    con.executemany(
        "INSERT INTO blobs VALUES (?,?)", [(f"{rel}-{i}", b) for i, b in enumerate(blobs)]
    )
    con.commit()
    con.close()
    return d
...
class WireFormat(unittest.TestCase):
    def test_reads_the_shapes_cursor_writes(self):
        ...
        self.assertEqual(list(_fields(msg)), [(1, 2, b"hello"), (26, 0, START_MS)])
    ...
    def test_step_clocks_are_sorted_and_filtered(self):
        self.assertEqual(_step_clocks(_step_blob(STEP2_MS, STEP_MS)), [STEP_MS, STEP2_MS])
        # a varint that is not a plausible ms epoch is not a clock
        self.assertEqual(
            _step_clocks(_len_field(2, _len_field(57, b"c") + _varint_field(59, 42))), []
        )
...
class Adapter(unittest.TestCase):
    ...
    def test_tool_steps_keep_their_real_delta(self):
        deltas = [e.time.delta_prev_s for e in self.events if e.data_type is DataType.TOOL_EVENT]
    ...
    def test_a_thread_with_no_anchor_invents_none(self):
        with tempfile.TemporaryDirectory() as t:
            root = Path(t)
            _make_store(root, "ws/x", [_json_blob("user", "hello there friend")], meta_json=False)
