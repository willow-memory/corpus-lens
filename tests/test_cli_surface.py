...
def _ev(thread, day, delta=None, chars=40, author=AuthorClass.OPERATOR, dtype=DataType.PROMPT):
    return Event(
        event_id="e",
        corpus_id="c",
        adapter_id="a",
        ...
        author_class=author,
        data_type=dtype,
        time=CoarseTime(day_offset=day, delta_prev_s=delta),
...
class WindowTests(CorpusFixture):
    ...
    def test_until_day_is_inclusive(self):
        _, out, _ = _run(
            ["run", str(self.d), "--adapter", "claude-code", "--format", "json", "--until-day", "0"]
        )
    ...
    def test_empty_window_is_an_error_not_an_empty_report(self):
        rc, out, err = _run(["run", str(self.d), "--adapter", "claude-code", "--since-day", "99"])
    ...
    def test_inverted_window_is_rejected_before_ingest(self):
        rc, _, err = _run(
            ["run", str(self.d), "--adapter", "claude-code", "--since-day", "5", "--until-day", "2"]
        )
...
class DoctorTests(CorpusFixture):
    def test_doctor_counts_match_the_corpus(self):
        rc, out, _ = _run(["doctor", str(self.d), "--adapter", "claude-code", "--format", "json"])
        self.assertEqual(rc, 0)
        diag = json.loads(out)
    ...
    def test_doctor_output_goes_through_the_egress_backstop(self):
        ...
        with mock.patch.object(cli, "_render_doctor", return_value="corpus starts 2026-02-01"):
