...
class CoarsenResultTests(unittest.TestCase):
    def test_unknown_fields_are_excluded_by_default(self):
        ...
        res = {
            ...
            "n": 500,
            "denominator": "turns",
            "brand_new_shape_field": 42,
            "concurrency_something_pct": 77.7,
    ...
    def test_only_allowlisted_rate_fields_survive(self):
        res = {
            "n": 40,
            "denominator": "turns",
            "mid_task_share_pct": 62.0,
            "opener_median_words": 11,
            "reference": {"x": 1},
        }
        out = coarsen_result(res)
...
class RateFieldsAreReviewedNotGuessedTests(unittest.TestCase):
    def test_shape_shaped_pct_fields_are_deliberately_excluded(self):
        ...
        for shape_field in (
            "single_turn_sessions_pct",
            "single_day_threads_pct",
            "burst_pct",
            "resumed_pct",
        ):
    ...
    def test_every_non_allowlisted_numeric_field_is_absent_from_share_output(self):
        ...
        for name, res in full.items():
            ...
            for key, val in res.items():
                ...
                self.assertNotIn(key, share[name], f"{name}.{key} leaked into share output")
        ...
        self.assertTrue(
            checked_any,
            "no numeric shape field found to check — "
...
class AuditBandingTests(CorpusFixture):
    ...
    def test_coarsen_audit_bands_all_three_counts_and_the_sentence_together(self):
        audit = AuditRecord(
            ...
            n_dropped=7,
            adapter="claude-code",
            filters=["relative-day window 2..corpus end, inclusive"],
            n_filtered=13,
        )
    ...
    def test_cli_share_output_bands_n_events_and_n_dropped(self):
        ...
        m = re.search(
            r"This run read (\S+) events \(dropped (\S+): (\S+) not a turn by "
    ...
    def test_cli_share_output_bands_n_filtered_on_a_windowed_run(self):
        _, full_out, _ = _run(
            ["run", str(self.d), "--adapter", "claude-code", "--format", "json", "--since-day", "2"]
        )
        _, share_out, _ = _run(
            [
                "run",
...
class CoarsenIsPureTests(CorpusFixture):
    def test_coarsen_does_not_mutate_its_input(self):
        ...
        results = {a.name: {"denominator": a.denominator, **a.run(events)} for a in all_analyzers()}
    ...
    def test_markdown_renders_the_coarsened_dict_without_error(self):
        ...
        coarsened = coarsen({"a": {"n": 40, "denominator": "turns", "mid_task_share_pct": 61.9}})
